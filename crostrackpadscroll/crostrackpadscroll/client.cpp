...
BOOL crostp_connect(pcrostp_client crostp)
{
	...
	crostp->hMessage = SearchMatchingHwID(0xff00, 0x0001);
	if (crostp->hMessage == INVALID_HANDLE_VALUE || crostp->hMessage == NULL)
	{
		crostp_disconnect(crostp);
		return FALSE;
	}

	crostp->hControl = SearchMatchingHwID(0xff00, 0x0002);
	if (crostp->hControl == INVALID_HANDLE_VALUE || crostp->hControl == NULL)
	{
		crostp_disconnect(crostp);
		return FALSE;
	}
	...
	if (!HidD_SetNumInputBuffers(crostp->hMessage, 10))
	{
	...
	if (!HidD_SetNumInputBuffers(crostp->hControl, 10))
	{
		printf("failed HidD_SetNumInputBuffers %d\n", GetLastError());
		crostp_disconnect(crostp);
		return FALSE;
...
BOOL crostp_connect2(pcrostp_client crostp) {
	...
	//
	// Set the buffer count to 10 on the settings HID
	//
...
void CrosTpClientChangeSetting(pcrostp_client crostp, int settingRegister, int settingValue) {
	...
	crostp_write_setting(crostp, &settingsReport);
}

void crostp_disconnect(pcrostp_client crostp)
{
	if (crostp->hMessage != NULL)
		CloseHandle(crostp->hMessage);
	crostp->hMessage = NULL;

	if (crostp->hControl != NULL)
		CloseHandle(crostp->hControl);
	crostp->hControl = NULL;
}
...
BOOL crostp_read_message(pcrostp_client crostp, CrosTpScrollReport* pReport)
{
	...
	if (!ReadFile(crostp->hMessage, pReport, sizeof(CrosTpScrollReport), &bytesRead, NULL))
	{
...
BOOL crostp_write_message(pcrostp_client crostp, CrosTpScrollControlReport* pReport)
{
	...
	if (!WriteFile(crostp->hControl, pReport, sizeof(CrosTpScrollControlReport), &bytesWritten, NULL))
	{
